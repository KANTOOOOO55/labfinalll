...
std::map<QString, Client> Client::getAllClients() // function to get all details of all clients reads all client information and the key is the client's name and the value is the client

{
    ...
    QTextStream in(&file);
    while (!in.atEnd()) {    //  loops through the file line by line
...
void Client::saveClient(Client client)  //function to save client
{
    QString filePath = QDir::homePath() + "/Documents/clients.txt";
    QFile file(filePath);
...
void Client::saveAllClients(std::map<QString, Client> clients)  //function to save all clients
{
    QString filePath = QDir::homePath() + "/Documents/clients.txt";
    QFile file(filePath);
